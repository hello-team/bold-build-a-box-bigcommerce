...
export default function SideCart(props) {
    ...
    const getSubs = (cartItems, cartId) => {
        ...
        subscriptionItems.length !== 0 ? Promise.all(subscriptionItems.map((item, index) => {
            ...
            let subscription_group_id = product.custom_fields.filter(field => field.name === "subscription_group_id")[0].value
            let interval_id = product.custom_fields.filter(field => field.name === "interval_id")[0].value
            let interval_text = product.custom_fields.filter(field => field.name === "interval_name")[0].value
    ...
    return (
        <div >
            ...
            >
                ...
                <Form action={checkoutData.url} method={'post'} >
